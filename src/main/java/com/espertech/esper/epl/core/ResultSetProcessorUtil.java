...
public class ResultSetProcessorUtil {
    public static void applyAggClearResult(AggregationService aggregationService, ExprEvaluatorContext exprEvaluatorContext, EventBean[] newData, Object... groupByKeys) {
        if (FeatureToggle.isDiscardExtTimedWindowOnAggOutput()) {
            if (exprEvaluatorContext instanceof AgentInstanceContext) {
                Set<ExternallyTimedWindowView> extTimedWindowViews = ((AgentInstanceContext) exprEvaluatorContext).getExtTimedWindowView();
                if (extTimedWindowViews.size() == 1) {
                    // Externally
                    for (ExternallyTimedWindowView extTimedWindowView : extTimedWindowViews) {
                        extTimedWindowView.applyClear(groupByKeys);
                    }
                } else if (extTimedWindowViews.size() > 1) {
                    throw new RuntimeException("Unexpected ExternallyTimedWindowView size in AgentInstanceContext");
                }
            } else {
                throw new RuntimeException("Unsupported ExprEvaluatorContext type: " + exprEvaluatorContext.getClass().getCanonicalName());
            }
        }
